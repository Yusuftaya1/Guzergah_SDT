...
class MotorController(Node):
    ...
    def angle_callback(self, msg):
        ...
        if w != 0.0:
            hiz_sol = linear + (w * (self.wheel_distance / 2.0)) # sol (-) sag (+) idi sorun olursa geri al 
            hiz_sag = linear - (w * (self.wheel_distance / 2.0)) # msg.sol = sag_ , msg. sag = sol_
        else:
            hiz_sol = linear
            hiz_sag = linear
        ...
        self.motor_values_msg.sag_teker_hiz = (1000/0.23)*hiz_sag_angular
        self.motor_values_msg.sol_teker_hiz = (1000/0.23)*hiz_sol_angular
